...
public class GenericDao {
	...
	public static synchronized GenericDao getInstance() {
		synchronized (GenericDao.class) {
			if (instance == null) {
				...
				instance = new GenericDao(keyspaceName, clusterName,
						clusterLocation);
			}
		}
		return instance;
	...
	private GenericDao(String keyspaceName, String clusterName,
	...
	protected void createKeyspace(String keyspaceName) {
		...
		List<ColumnFamilyDefinition> definicoes = new ArrayList<ColumnFamilyDefinition>();
		ColumnFamilyDefinition countDef = HFactory
				.createColumnFamilyDefinition(keyspaceName, "contagem",
		...
		KeyspaceDefinition novoKeyspace = HFactory.createKeyspaceDefinition(
				keyspaceName, ThriftKsDef.DEF_STRATEGY_CLASS, 1, definicoes);
